...
export default class BookConfirm extends React.PureComponent {
        ...
        if (result.status == 1) {
          ...
          if (item.id == 1) {
            ...
            surchnages =
              ...
              parseFloat(minimumfare * (parseFloat(gstper) / 100));
          } else {
            surchnages =
              parseFloat(result.data.fuel_surge_charge) +
              parseFloat(result.data.nsw_gtl_charge) +
          ...
          <Row size={45}>
            <Col>
              <ScrollView horizontal={true}>
                {Object.keys(this.state.servicetypes).length > 0 &&
                  this.state.servicetypes.map((item, index) => {
                    if (faresmap.length > 0 && faresmap[item.id]) {
                      return (
                        ...
                        >
                          <TouchableOpacity
                            onPress={() => this._onPressService(item, faresmap)}
  ...
  async intialLoad() {
    ...
    if (this.props.route.params.inputData) {
      console.log(
        "IN INPUT data" + Object.keys(this.props.route.params.inputData).length
      );
      ...
      <View style={styles.container}>
        ...
        <MapboxGL.MapView
          ...
          onDidFinishRenderingMapFully={(index) => {
            ...
            fetch(
              "https://api.mapbox.com/directions/v5/mapbox/driving/79.075274,21.089974;79.0899,21.0893;79.1069,21.0962;79.097723,21.131447?geometries=geojson&access_token=sk.eyJ1IjoiamttaWxsYSIsImEiOiJjbGlzM3JtMmswczN5M2NxdmR2cDB3bTNuIn0.vBo7vBXjCnpxsyHAwe4jZw",
              {
                method: "GET",
              }
            )
