...
export default class RideConfirm1 extends React.Component {
  ...
  listenForChanges = () => {
    ...
    channel.bind("notify_rider_remain_time", (data) => {
      ...
      if (
        data.rider_id == this.state.bookingresponse.rider_id &&
  ...
  renderDriverLocation = () => {
    if (
      ...
      this.state.driverLocation.latitude != null &&
      this.state.driverLocation.longitude != null
    ) {
      let rotateAngl = 0;
  ...
  async getrideStart() {
    ...
    firestore()
      ...
      .where("bookingId", "==", this.state.bookingresponse.id)
      .get()
      .then((querySnapshot) => {
        ...
        if (querySnapshot.size > 0) {
          ...
          if (this.state.tuneTripStart) {
            this.state.tuneTripStart.stopAsync();
            this.state.tuneTripStart.playAsync();
          }
          this.setState(
            {
              tripstart: true,
      ...
      fetch(
        ...
        .then((json) => {
          ...
          if (json.message == "cancel") {
            ...
            if (this.state.tuneBookingCancelByDriver) {
              this.state.tuneBookingCancelByDriver.stopAsync();
              this.state.tuneBookingCancelByDriver.playAsync();
            }
            clearInterval(this.myinterval);
            clearInterval(this.settime);
            setTimeout(
  ...
  async getLnglatdriver2source(dLocation) {
    ...
    fetch(
      "https://api.mapbox.com/directions/v5/mapbox/driving/" +
        loctstring +
        "?overview=full&geometries=geojson&access_token=sk.eyJ1IjoiamttaWxsYSIsImEiOiJjbGlzM3JtMmswczN5M2NxdmR2cDB3bTNuIn0.vBo7vBXjCnpxsyHAwe4jZw",
      {
        method: "GET",
      }
    )
  ...
  async updatecarLoc() {
    ...
    this.settime = setInterval(() => {
      ...
      var docRef = firestore()
        ...
        .doc(JSON.stringify(this.props.route.params.bookingdriver.id));
      docRef
        .get()
        .then((doc) => {
          if (doc.exists) {
            ...
            this.setState(
              {
                driverLocation: doc.data().coordinates,
  ...
  createSoundObject = async () => {
    ...
    const { sound: tuneMessageFromDriver } = await Audio.Sound.createAsync(
      require("../assets/tunes/text_message_from_driver.mp3"),
      {},
      this.onPlaybackSoundObject
    );

    const { sound: tuneTripStart } = await Audio.Sound.createAsync(
      require("../assets/tunes/trip_start.mp3"),
      {},
      this.onPlaybackSoundObject
    );

    const { sound: tuneBookingCancelByDriver } = await Audio.Sound.createAsync(
      require("../assets/tunes/booking_cancel.mp3"),
      {},
      this.onPlaybackSoundObject
    );
    ...
    this.setState(
      {
        ...
        tuneMessageFromDriver,
        tuneTripStart,
        tuneBookingCancelByDriver,
      },
      () => {
  ...
  componentDidMount() {
    ...
    if (this.props.route.params.selectedvehicle) {
      let driver_name =
        this.props.route.params.bookingdriver.first_name +
        " " +
        this.props.route.params.bookingdriver.last_name;
        ...
        if (doc.exists) {
          ...
          this.setState(
            ...
            () => {
              ...
              const destheading = this.getHeading(
                this.state.origin,
  ...
  renderMessages = () => {
    return (
      ...
      >
        ...
        >
          Your driver arrives in {this.state.duration} minutes
        </Text>
      </View>
    );
  };
    ...
    return (
      ...
      >
        ...
        <Button
          mode="contained"
          color={"#135AA8"}
          onPress={() => this.setState({ inmessagevisible: true })}
        >
          Open
        </Button>
      </View>
          ...
          <Grid>
            <Row style={{ height: "30%" }}>
              <Col size={3.5} style={{ padding: 3 }}>
                <TouchableHighlight
                  onLongPress={() => {
                    this._onLongPressButton(this.state.bookingdriver.avatar);
                  }}
                  underlayColor="white"
                >
              ...
              <Col size={6} style={{ padding: 3 }}>
                <Text
                  style={{ fontSize: 18, color: "#135AA8", fontWeight: "bold" }}
                >
                  {this.state.bookingdriver.first_name}{" "}
                  {this.state.bookingdriver.last_name}
                </Text>
                <Text
                  style={{ fontSize: 12, fontWeight: "bold", color: "#135AA8" }}
                >
                  Car Rego:{" "}
                  {this.state.vehinfo.plate ? this.state.vehinfo.plate : ""}
                </Text>
                <Text
                  style={{ fontSize: 12, fontWeight: "bold", color: "#135AA8" }}
                >
                  {this.state.vehinfo.makename
                    ? this.state.vehinfo.makename
                    : ""}{" "}
                  :{" "}
                  {this.state.vehinfo.modelname
                    ? this.state.vehinfo.modelname
                    : ""}{" "}
                </Text>
                <Text
                  style={{ fontSize: 12, fontWeight: "bold", color: "#135AA8" }}
                >
                  {this.state.vehinfo.color ? this.state.vehinfo.color : ""} ,{" "}
              ...
              <Col size={4} style={{ padding: 3 }}>
                <TouchableHighlight
                  onLongPress={() => {
                    this._onLongPressButton(this.state.vehinfo.front_photo);
                  }}
                  underlayColor="white"
                >
            ...
            <Row style={{ height: 50 }}>
              ...
              <Col
                size={3}
                style={{ alignItems: "flex-end", margin: 8, height: 55 }}
              >
                <View
                  style={{
                    flex: 1,
                ...
                >
                  <TouchableOpacity
                    onPress={() => {
                      this.setState({ inmessagevisible: true });
                    }}
                  >
                    <AntDesign
                      name="message1"
            ...
            <Row style={{ height: 40, padding: 6, marginTop: 20 }}>
              <Col size={1}>
              ...
              <Col size={11}>
                <Text
                  style={{ color: "#3f78ba", fontWeight: "bold", fontSize: 15 }}
                >
                  {" "}
                  Long press on image to zoom in
                </Text>
              </Col>
            </Row>
          </Grid>
    ...
    this.setState(
      {
        chatMessage: [
          ...
          ...[{ body: text, from: "rider" }],
        ],
      },
      () => {
        AsyncStorage.setItem(
          "chatMessage",
          JSON.stringify(this.state.chatMessage)
        );
  ...
  riderMessages = async () => {
    messaging().onMessage(async (remoteMessage) => {
      ...
      // await this.stopRunSound();
      if (this.state.tuneMessageFromDriver) {
        await this.state.tuneMessageFromDriver.stopAsync();
        await this.state.tuneMessageFromDriver.playAsync();
      }
    ...
    return (
      <View style={styles.container}>
        ...
        <FlashMessage
          ...
          position={{ top: 80, left: 10, right: 10 }}
        ...
        <MapboxGL.MapView
          ...
          onDidFinishRenderingMapFully={(index) => {
            ...
            fetch(
              "https://api.mapbox.com/directions/v5/mapbox/driving/" +
                this.state.locationcordsapistr +
                "?overview=full&geometries=geojson&access_token=sk.eyJ1IjoiamttaWxsYSIsImEiOiJjbGlzM3JtMmswczN5M2NxdmR2cDB3bTNuIn0.vBo7vBXjCnpxsyHAwe4jZw",
              {
                method: "GET",
              }
              ...
              .then((result) => {
                ...
                this.setState(
                  ...
                  () => {
                    if (
                      this.camera &&
                      this.state.longitudedest != "" &&
                      this.state.latitudedest != ""
                    ) {
                      this.camera.fitBounds(
                        ...
                        [this.state.longitudedest, this.state.latitudedest],
                        [150, 40, 250, 20],
                        1000
                      );
                      /*this.camera.setCamera({
        ...
        >
          ...
          <MapboxGL.ShapeSource
            id="mapbox-directions-driver"
            shape={this.state.routediver}
          >
            <MapboxGL.LineLayer
            ...
            >
              ...
              >
                ...
                >
                  <Image
                    source={{
                      uri: "https://www.turvy.net/" + this.state.imagezoom,
                    }}
                    style={styles.servicebocimage}
                    Resizemode={"contain"}
                  />
        ...
        >
          <InMessages
            ...
            strAsynChatMessage={this.strAsynChatMessage}
            addChatMessage={this.addChatMessage}
            closeAddastop={this.closeAddastop}
          />
        </Modal>

        {this.state.alert5min && this.renderDriverMessage("5")}

        {this.state.alert15min && this.renderDriverMessage("15")}

        {this.state.alertCongrats && this.renderMessagesDriver()}
        {this.state.alert1min && this.render1minDriverMessage("1")}
      </View>
    );
  }
}
