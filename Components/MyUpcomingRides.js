  ...
  getMyRider = async () => {
    await AsyncStorage.getItem("accesstoken").then((value) => {
      ...
      fetch("https://www.turvy.net/api/rider/myrides/1/" + this.state.pageno, {
        ...
        .then((result) => {
          console.log(
            "UPCOMMING TRIPS PAGE - " + this.state.pageno,
            this.state.trips
          );
      ...
      <>
        <View>
          <Collapse
            ...
            isExpanded={this.state.selectedkeyid == item.item.id ? true : false}
            onToggle={(isExpanded) => {
              if (isExpanded) {
                this.setState({ selectedkeyid: item.item.id });
              } else {
                ...
                <Row size={50}>
                  ...
                  <Col size={5}>
                    <Text style={{ fontSize: 14 }}>
                      {item.item.booking_date}, {item.item.booking_time}
                    </Text>
                  </Col>
                  <Col size={5}>
              ...
              <Row style={{ width: "100%", flex: 1, flexDirection: "row" }}>
                <Col size={12}>
                  {item.item.first_name || item.item.last_name ? (
                    <Text
                      style={{ padding: 10, fontWeight: "bold", fontSize: 14 }}
                    >
                      Your {item.item.servicename} Trip with{" "}
                      {item.item.first_name} {item.item.last_name}
                    </Text>
                  ) : (
                    <Text
                      style={{ padding: 10, fontWeight: "bold", fontSize: 14 }}
                    >
                      Your {item.item.servicename} Trip{" "}
                    </Text>
                  )}
                </Col>
              </Row>
              <Row style={{ width: "100%", flex: 1, flexDirection: "row" }}>
                ...
                <Col size={10}>
                  <Row size={12} style={{ height: 30 }}>
                    <Col size={12}>
                      <Text
                        style={{ paddingTop: 8, fontSize: 16 }}
                        numberOfLines={1}
                      >
                        {item.item.origin}
                      </Text>
                  ...
                  <Row size={12} style={{ height: 30 }}>
                    <Col size={12}>
                      <Text
                        style={{ fontSize: 15, color: "grey", paddingTop: 10 }}
                      >
              ...
              >
                ...
                <Col size={4}>
                  ...
                  >
                    ...
                    >
                      Details{" "}
                      <AntDesign name="arrowright" size={24} color="#3f78ba" />
                    </Text>
                  </TouchableOpacity>
                </Col>
              </Row>
      ...
      <PaperProvider theme={theme}>
        ...
        <View>
          {this.state.runningtripId && this.state.runningtripId > 0 ? (
            <View style={{ width: "40%", marginTop: 20, marginLeft: 10 }}>
              <Button
                mode="contained"
                color={"#135AA8"}
                onPress={() => {
