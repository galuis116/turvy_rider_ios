...
const SCREENHEIGHT = height * 0.66;
const faresmap = [];
const basechnagre = [];
const perunitcharge = [];
const cancelcharge = [];
...
export default class BookConfirm extends React.PureComponent {
  ...
  */
    ...
    fetch("https://www.turvy.net/api/farecard/2/" + item.id, {
      ...
      .then((result) => {
        ...
        if (result.status == 1) {
          ...
          const minimumfare =
            parseFloat(result.data.price_per_unit) *
            parseFloat(this.state.distance);
          let gstper = result.data.gst_charge;
          gstper = gstper.replace("%", "");
          let nsw_ctp =
            parseFloat(result.data.nsw_ctp_charge) *
            parseFloat(this.state.distance);
          let fuel_surc =
            parseFloat(result.data.fuel_surge_charge) *
            parseFloat(this.state.distance);
          //alert(gstper);
          //	if()
        ...
        >
          <Grid>
            <Row size={45}>
              <Col>
                      ...
                      if (faresmap.length > 0 && faresmap[item.id]) {
                        return (
                          ...
                          >
                            <TouchableOpacity
                              onPress={() =>
                                this._onPressService(item, faresmap)
                              }
                              onLongPress={() => this._onLongPressButton(item)}
                              delayLongPress={500}
                            >
                              <Image
                                source={{
                                  uri: "https://www.turvy.net/" + item.image,
                                }}
                ...
                {this.state.fareavilable ? (
                  ...
                  >
                    Choose a ride or scroll for more ...
                  </Text>
                ) : (
                  <View>
                    <Text style={{ fontWeight: "bold", color: "red" }}>
                      Pickup and destination should not same or Fare info is not
                      available!
                    </Text>
                  </View>
                )}
            ...
            >
              ...
              <Col size={11}>
                <Text
                  style={{ color: "#135aa8", fontWeight: "bold", fontSize: 11 }}
                >
                  {" "}
                  Long press on image to get approximate break-down fare.
                </Text>
              </Col>
            </Row>
            <Row size={15} style={{ justifyContent: "center" }}>
              ...
              >
                <TouchableOpacity
                  onPress={() => this.setState({ scheduleboxvisible: true })}
                >
                  <View
                    style={{
                      justifyContent: "center",
  ...
  async intialLoad() {
    ...
    if (Object.keys(this.props.route.params.origin).length > 0) {
      //console.log("origin from===============", this.props.route.params.origin);
      let origincord = [
  ...
  getseatcost = () => {
    if (this.state.longpressdata.baby_seat_charge > 0) {
      return (
        <Row size={6}>
          <Col size={75}>
            <Text style={styles.label}>Baby Seat Charge </Text>
          </Col>
          <Col size={25}>
            <Text style={styles.label}>
              A${this.state.longpressdata.baby_seat_charge}
            </Text>
          </Col>
        </Row>
      );
      ...
      return (
        <Row size={6}>
          <Col size={75}>
            <Text style={styles.label}>Pet Charge</Text>
          </Col>
          <Col size={25}>
            <Text style={styles.label}>
              A${this.state.longpressdata.pet_charge}
            </Text>
          </Col>
        </Row>
      );
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
              ...
              .then((result) => {
                ...
                if (Object.keys(this.state.fares).length) {
                  ...
                  this.state.fares.map((item, key) => {
                    ...
                    basechnagre[item.servicetype_id] =
                    ...
                    gstper = gstper.replace("%", "");
                    let nsw_ctp =
                      parseFloat(item.nsw_ctp_charge) * parseFloat(distance);
                    let fuel_surc =
                      parseFloat(item.fuel_surge_charge) * parseFloat(distance);
                    //alert(fuel_surc);
          ...
          {Object.keys(this.state.drivernear).length > 0
            ? this.state.drivernear.map((marker, index) => {
                return (
                  ...
                  >
                    <Image style={styles.vehmarkerimage} source={imageveh} />
                  </MapboxGL.PointAnnotation>
                );
              })
            : null}
        ...
        <ActivityIndicator animating={false} size="large" color="#135AA8" />
        {this.state.display ? (
          <BottomSheet
                ...
                >
                  ...
                  >
                    ...
                    <Row size={6}>
                      ...
                      <Col size={25}>
                        <Text style={styles.label}>
                          A${this.state.longpressdata.base_ride_distance_charge}{" "}
                        </Text>
                      </Col>
                    </Row>
                    ...
                    <Row size={6}>
                      ...
                      <Col size={25}>
                        <Text style={styles.label}>
                          A${this.state.longpressdata.price_per_ride_minute}
                        </Text>
                      </Col>
                    </Row>
                    ...
                    <Row size={6}>
                      ...
                      <Col size={25}>
                        <Text style={styles.label}>
                          A${this.state.longpressdata.price_per_unit}
                        </Text>
                      </Col>
                    </Row>
                    <Row size={6}>
                      <Col size={75}>
                        <Text style={styles.label}>Estimated Surcharges</Text>
                      </Col>
                      <Col size={25}>
                        <Text style={styles.label}>
                          A${this.state.surchnages}{" "}
                        </Text>
                      </Col>
                    </Row>
                    <Row size={6}>
                      ...
                      <Col size={25}>
                        <Text style={styles.label}>
                          A${this.state.longpressdata.booking_charge}{" "}
                        </Text>
                      </Col>
                    </Row>
                    <Row size={6}>
                    ...
                    <Row size={20}>
                      <Col size={12}>
                        ...
                        >
                          Your fare will be price presented before the trip base
                          on the rate above and other applicable surcharges and
                          adjustments. Additional waiting charges may apply to
                          your trip if the driver has waited{" "}
                          {this.state.longpressdata.fee_waiting_time == 0
                            ? ""
                            : this.state.longpressdata.fee_waiting_time}{" "}
                          A${this.state.longpressdata.waiting_price_per_minute}{" "}
                          per minutes.
                        </Text>
                      </Col>
                    </Row>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <AntDesign name="closecircle" size={30} color="#135AA8" />
                    </TouchableOpacity>
                  </View>
                  <View
                    style={{
