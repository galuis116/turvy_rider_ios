...
export default class BookProcess extends React.Component {
      ...
      async (location) => {
        ...
        if (this.state.airport_Cord.length > 0) {
          for (let k = 0; k < this.state.airport_Cord.length; k++) {
            ...
            res = geolib.isPointInPolygon(
              {
                latitude: location.coords.latitude,
                longitude: location.coords.longitude,
              },
              this.state.airport_Cord[k].coords
            );
            if (res) {
              airportInfo = {
  ...
  assignDriverIndiv = async (driver_id) => {
    ...
    fetch(
      "https://www.turvy.net/api/rider/assigndriver/" +
        this.state.bookingresponse.id,
      {
        method: "POST",
        headers: new Headers({
  ...
  componentDidMount() {
    ...
    if (this.props.route.params.selectedvehicle) {
      AsyncStorage.setItem(
        "last_booking_id",
        JSON.stringify(this.props.route.params.bookingresponse.id)
      );
      this.setState(
        {
  ...
  declineBydriver = async (driverAId) => {
    ...
    console.log(accesstoken);
    fetch(
      "https://www.turvy.net/api/rider/getdeclinedriverreq/" +
        this.state.bookingresponse.id,
      {
        method: "POST",
        headers: {
      ...
      <View style={styles.container}>
        ...
        >
          <Grid style={{ justifyContent: "center", alignContent: "center" }}>
              ...
              >
                ...
                >
                  <Text style={{ fontSize: 20, fontWeight: "bold" }}>
                    Are you sure you want to go back, It will Cancel current
                    trip request?
                  </Text>
                  <Row
                    size={10}
            ...
            <Row size={10} style={{ marginTop: 10 }}></Row>
            <Row size={40}>
              <Col></Col>
            </Row>
          </Grid>
