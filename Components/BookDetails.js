...
const SCREENHEIGHT = height * 0.65;
...
export default class BookDetails extends React.PureComponent {
  ...
  async getNearBydriver() {
    ...
    const query = geocollection.near({
      center: new firebase.firestore.GeoPoint(-33.8688, 151.2195),
      radius: 100,
    });
    //.where('isBusy','==','no')
  ...
  async _retry() {
    await AsyncStorage.getItem("accesstoken").then((value) => {
      fetch(
        "https://www.turvy.net/api/rider/book/retry/" +
  ...
  async _onPressDone() {
    ...
    await AsyncStorage.getItem("accesstoken").then((value) => {
      fetch(
        "https://www.turvy.net/api/rider/book/cancel/" +
          this.state.bookingresponse.id,
        {
          method: "POST",
          headers: new Headers({
            ...
            <Row size={3}>
              <Col>
                ...
                >
                  ...
                  >
                    Cab not available on your location or driver not responding.
                  </Text>
                  <Text
                    style={{
                      color: "#fff",
