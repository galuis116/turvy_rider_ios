            ...
            .then((result) => {
              ...
              if (result.access_token != "") {
                try {
                  ...
                  if (
                    result.rider.avatar != "" ||
                    result.rider.avatar != null
                  ) {
                    AsyncStorage.setItem(
                      "avatar",
                      DOMAIN + result.rider.avatar
                    );
                  }

                  AsyncStorage.setItem(
                    "partner_id",
                    JSON.stringify(result.rider.partner_id)
                  );
                  AsyncStorage.setItem(
                    "rider_id",
                    JSON.stringify(result.rider.id)
                  );
                  AsyncStorage.setItem("countrycode", countrycode);
                  AsyncStorage.setItem("phone", phone).then((res) => {
    ...
    <SafeAreaView style={stylesv.root} keyboardShouldPersistTaps="handled">
      ...
      >
        ...
        <View style={stylesv.verificationBox}>
          <Text style={stylesv.heading}>
            We sent OTP code to Verify your number
          </Text>
          <Text style={{ color: "red", fontSize: 12 }}>{error}</Text>
          <View>
            <Text style={stylesv.h6}>Enter OTP Below</Text>
          </View>
          <View
            style={{
          ...
          >
            <Text style={stylesv.textUnique}>
              If you didnt receive a code?{" "}
            </Text>
            <TouchableHighlight onPress={() => resendOtp()}>
              <Text style={[stylesv.textUnique, stylesv.textBlue]}>
                Resend OTP
              </Text>
            </TouchableHighlight>
          </View>
          ...
          >
            <LinearGradient
              ...
              colors={
                disabled ? ["#74c0ee", "#74c0ee"] : ["#2270b8", "#74c0ee"]
              }
              end={{ x: 1.2, y: 1 }}
            >
              <View
              ...
              >
                <Text
                  style={[styles.priBtnTxt, { flex: 7, textAlign: "center" }]}
                >
                  Verify
                </Text>
              </View>
