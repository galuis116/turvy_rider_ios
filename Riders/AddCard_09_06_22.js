 ...
 componentDidMount(){
  ...
  getTokenlist = async() =>{
	  	...
	  	fetch('https://www.turvy.net/api/rider/getPaymentToken',{
				...
				headers: new Headers({
	 			...
	 			return response.json();
	  			...
	  			let cardsinfoList = [];
	  			console.log("Paymnet Token List ",result);
	  			if(result.data){
   ...
   async submit (){
    ...
    if( this.state.cardInfo.status.cvc == 'incomplete' || this.state.cardInfo.status.expiry == 'incomplete' || this.state.cardInfo.status.name == 'incomplete' || this.state.cardInfo.status.number == 'incomplete'){
    	this.refs.fmLocalIntstance.showMessage({
           message: 'Please input card details',
           type: "danger",
           style:{
    ...
    render() {
        return (
            ...
            <Appbar.Header style={{backgroundColor:'#fff'}}>
                ...
                <View>
                <Text style={{fontSize:20,padding:10}}>Choose Card Or Add new Card</Text>
			    	...
			    	<View style={{'backgroundColor':'#fff',padding:10,borderRadius:1,borderBottomWidth:1,borderBottomColor:'grey'}}>
			    	 <TouchableOpacity onPress={() => { this.setState({checked:item.card.id}) }} >
			    	 	<View>
			    	 	 <Grid>
			    	 	 	<Row style={{padding:6}}>
			    	 	 	 <Col size={2}>
                        ...
                        resizeMode="cover" />
			    	 		...
			    	 		<Row style={{padding:6}}>
			    	 	 	 <Col size={3}>
			    	 			<Text style={{fontWeight:'bold',}}>Expire on</Text>
			    	 		</Col>
			    	 		<Col size={8}>
			    	 			<Text style={{fontWeight:'bold',}}>{item.card.exp_month}/{item.card.exp_year} </Text>
			    	 		</Col>
			    	 		<Col size={1}>
			    	 			{(this.state.checked === item.card.id)
			                        ?
					                	<Checkbox 
					                		style={s.radio} 
					                		value={item.card.id} 
					                		status={ this.state.checked === item.card.id ? 'checked' : 'unchecked' }
					                		color='#598cc3'
					                		uncheckedColor='#598cc3'
					                		onPress={() => { this.setState({checked:item.card.id	}) }}
					                	/>
			                      :
			                      <></>
			                      }
			       ...
			       ( <TouchableHighlight             
                        ...
                        this.fetchPaymentIntentClientSecret(this.state.checked) } }}>
